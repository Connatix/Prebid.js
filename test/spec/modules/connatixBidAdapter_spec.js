...
import { spec, getBidFloor as connatixGetBidFloor } from '../../../modules/connatixBidAdapter.js';
...
describe('connatixBidAdapter', function () {
  ...
  describe('buildRequests', function () {
    ...
    this.beforeEach(function () {
      bid = mockBidRequest();
      serverRequest = spec.buildRequests([bid], bidderRequests);
    })
    ...
    it('Validate request payload', function () {
      ...
      expect(serverRequest.data.refererInfo).to.equal(bidderRequests.refererInfo);
      expect(serverRequest.data.gdprConsent).to.equal(bidderRequests.gdprConsent);
      expect(serverRequest.data.uspConsent).to.equal(bidderRequests.uspConsent);
      expect(serverRequest.data.ortb2).to.equal(bidderRequests.ortb2);
    });
  });
